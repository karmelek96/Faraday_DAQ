...
class AD7190 {
    public:
        AD7190(uint8_t _CS_PIN);
        
        /* Writes data into a register. */
        void setRegisterValue(uint8_t _registerAddress, uint32_t _registerValue, uint8_t _bytesNumber, bool _modifyCS);

        /* Reads the value of a register. */
        uint32_t getRegisterValue(uint8_t _registerAddress, uint8_t _bytesNumber, bool _modifyCS);

        /* Checks if the AD7190 is present. */
        uint8_t init();

        /* Resets the device. */
        void reset();

        /* Set the operating mode of the AD7190. */
        void setPower(uint8_t _powerMode);

        /* Waits for RDY pin to go low. */
        void waitRdyGoLow();

        /* Selects channel to be enabled */
        void channelSelect(uint8_t _channel);

        /* Performs calibration to the specified channel */
        void calibrate(uint8_t _mode, uint8_t _channel);

        /* Selects the polarity of the conversion and the ADC input range. */
        void rangeSetup(uint8_t _polarity, uint8_t _range);

        /* Returns the result of a single conversion. */
        uint32_t singleConversion();

        /* Returns the result of a continuous conversion. */
        uint32_t continuousSingleRead();

        uint32_t continuousReadAvg(uint8_t _sampleNumber);

    private:
        uint8_t cs_pin;
        uint8_t spi_write(uint8_t* _data, uint8_t _bytesNumber);
        uint8_t spi_read(uint8_t* _data, uint8_t _bytesNumber);
        void spi_init();
};